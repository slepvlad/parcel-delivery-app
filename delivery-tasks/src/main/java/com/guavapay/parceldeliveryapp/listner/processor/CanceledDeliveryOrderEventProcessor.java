...
public class CanceledDeliveryOrderEventProcessor implements DeliveryOrderEventProcessor {

    private final DeliveryTaskService deliveryTaskService;

    @Override
    public boolean shouldProcess(DeliveryOrderEvent event) {
        return "CANCELED".equals(event.getStatus());
    }

    @Override
    public void precess(DeliveryOrderEvent event) {
        log.info("Receive event:{}", event);
        deliveryTaskService.cancelDelivery(event.getOrderId());
    }
}
