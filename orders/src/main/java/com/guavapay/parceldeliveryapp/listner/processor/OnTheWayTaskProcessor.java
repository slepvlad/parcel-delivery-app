...
public class OnTheWayTaskProcessor implements DeliveryTaskEventProcessor{

    private final DeliveryOrderService deliveryOrderService;
    @Override
    public boolean shouldProcess(DeliveryTaskEvent event) {
        return "ON_THE_WAY".equals(event.getStatus());
    }

    @Override
    public void precess(DeliveryTaskEvent event) {
        deliveryOrderService
                .changeStatus(event.getOrderId(), OrderStatus.ON_THE_WAY);
    }
}
